...
**0.42.0 起上游自带虚拟化**：消息列表、每条消息的内容块和工具组条目都由 Kimi 的 HistoryWindow 组件原生开窗，适配器不再重复处理这些列表，只回收折叠工具组 / 思考块的常驻挂载体；设置里的「常驻最近消息」在该版本不起作用（消息窗口由上游管理），面板计数显示为「块」。0.41.x 及更早版本的行为不变。
...
- 浏览器功能检查：16 项合成会话（历史恢复、工具组展开 / 折叠、闲置回收、流式更新、补齐历史、停用还原、面板拖动与边界钳制）+ 7 项 HistoryWindow 结构（CLI 0.42.0+ 上游开窗模式）+ 16 项面板检查。
