...
当前白名单覆盖 **kimi-code CLI 0.33.0–2.1.0 的 18 个前端构建**，最新加入 `/assets/index-CiJ6FDOC.js`（CLI 2.1.0）。版本到构建的完整映射见 [upstream-state.json](.github/upstream-state.json)。新增构建经过静态契约核对与合成浏览器夹具；各版本并未全部进行真实会话实测。服务端版本不等于前端版本；未知前端构建不会启用适配。
