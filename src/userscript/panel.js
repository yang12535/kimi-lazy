(() => {
  ...
  function savePos() {
    ...
    try { localStorage.setItem(POS_KEY, JSON.stringify({ fx: parseFloat(host.style.left) / window.innerWidth, fy: parseFloat(host.style.top) / window.innerHeight })); } catch { /* position persistence is optional */ }
  }
  function restorePos() {
    let pos;
    try { pos = JSON.parse(localStorage.getItem(POS_KEY) || 'null'); } catch { return; }
    if (!pos || typeof pos.fx !== 'number' || typeof pos.fy !== 'number') return;
    setPos(pos.fx * window.innerWidth, pos.fy * window.innerHeight);
  }
