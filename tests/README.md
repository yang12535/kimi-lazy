...
访问 `http://127.0.0.1:58791/fixture.html` 自动运行 13 项合成会话检查。页面内嵌用户脚本，用于验证首次渲染接入；保持页面可见直到显示结果。结果写入忽略提交的 `tests/last-browser-result.json`。
...
测试涵盖窗口大小、旧消息恢复、工具组恢复与折叠、可见内容保护、闲置回收、流式更新、补齐历史、停用还原、切换会话和错误状态。十分钟回收通过模拟时间验证。
