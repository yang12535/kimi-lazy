...
访问 `http://127.0.0.1:58791/fixture.html` 自动运行 20 项旧结构合成会话检查。追加 `?hw=off` 运行 20 项 HistoryWindow 默认关闭模式检查，追加 `?hw=1` 运行 10 项显式原生开窗检查。页面内嵌用户脚本，用于验证首次渲染接入；保持页面可见直到显示结果。结果写入忽略提交的 `tests/last-browser-result.json`。
